...
int main() {
  ...
  std::cout << "Is box empty? " << (box.is_empty() ? "Yes\n" : "No\n");

  // Check if the box contains a point
  Point point(2.0, 1.0);
  std::cout << "Does the box contain the point? " << (box.contains(point) ? "Yes\n" : "No\n");

  // Check if the box intersects another box
  Box other_box(Point(1.0, 1.0), Point(3.0, 2.0));
  std::cout << "Does the box intersect the other box? " << (box.intersects(other_box) ? "Yes\n" : "No\n");
  ...
  std::cout << "Minimum distance between boxes: " << min_distance_boxes << '\n';
  ...
  std::cout << "Maximum distance between boxes: " << max_distance_boxes << '\n';
