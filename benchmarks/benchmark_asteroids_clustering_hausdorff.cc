...
int main() {
  ...
  auto hausdorff_distance_fun = [](Box const &b1, Box const &b2) {
    ...
    Real distance_b1_b2{0.0}, distance_b2_b1{0.0};
    Real dx, dy, dz, tmp;
    // B1 -> B2
    for (Integer i{0}; i < 3 * (t_steps - 1); i += 3) {
      tmp = std::numeric_limits<Real>::max();
      for (Integer j{0}; j < 3 * (t_steps - 1); j += 3) {
        dx = std::max(0.0, std::max(b1_min(i + 0) - b2_max(j + 0),
                                    b2_min(j + 0) - b1_max(i + 0)));
        dy = std::max(0.0, std::max(b1_min(i + 1) - b2_max(j + 1),
                                    b2_min(j + 1) - b1_max(i + 1)));
        dz = std::max(0.0, std::max(b1_min(i + 2) - b2_max(j + 2),
                                    b2_min(j + 2) - b1_max(i + 2)));
        tmp = std::min(tmp, dx * dx + dy * dy + dz * dz);
      }
      distance_b1_b2 = std::max(distance_b1_b2, tmp);
    }
    // B2 -> B1
    for (Integer j{0}; j < 3 * (t_steps - 1); j += 3) {
      tmp = std::numeric_limits<Real>::max();
      for (Integer i{0}; i < 3 * (t_steps - 1); i += 3) {
        dx = std::max(0.0, std::max(b2_min(j + 0) - b1_max(i + 0),
                                    b1_min(i + 0) - b2_max(j + 0)));
        dy = std::max(0.0, std::max(b2_min(j + 1) - b1_max(i + 1),
                                    b1_min(i + 1) - b2_max(j + 1)));
        dz = std::max(0.0, std::max(b2_min(j + 2) - b1_max(i + 2),
                                    b1_min(i + 2) - b2_max(j + 2)));
        tmp = std::min(tmp, dx * dx + dy * dy + dz * dz);
      }
      distance_b2_b1 = std::max(distance_b2_b1, tmp);
    }
    return std::sqrt(std::max(distance_b1_b2, distance_b2_b1));
  };

  timer.tic();
  for (Integer i{0}; i < n_clusters; ++i) {
    clusters_distance[i] = tree.closest(*tree.box(i), n_neighbours, clusters[i],
                                        hausdorff_distance_fun);
    if (i % 100 == 0) {
      timer.toc();
  ...
  // Save the clusters
  std::ofstream asteroids_cluster(
      "asteroids_clustering_clusters_hausdorff.txt");
  for (Integer i{0}; i < n_clusters; ++i) {
