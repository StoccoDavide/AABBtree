 ...
 */
template <typename Real> class TicToc {
...
public:
  ...
  void toc() {
    ...
    this->m_elapsed_time = std::chrono::duration_cast<Time>(this->m_stop_time -
                                                            this->m_start_time);
  }
  ...
  Real elapsed_s() const {
    return static_cast<Real>(1.0e-6) * this->m_elapsed_time.count();
  }
  ...
  Real elapsed_ms() const {
    return static_cast<Real>(1.0e-3) * this->m_elapsed_time.count();
  }
...
static const double GM{
    0.0002959122082855911}; // Gravitational parameter of the Sun in AU^3/day^2

// Keplerian orbital elements
template <typename Real, typename Integer> struct Keplerian {
...
bool Parse(std::string const &fname,
           std::vector<Keplerian<Real, Integer>> &data, Integer const &n,
           bool reverse = false) {
  // Clear data
  data.clear();
  data.reserve(n);
  ...
  for (const auto &i_line : lines) {
    ...
    iss >> entry.id >> entry.epoch >> entry.a >> entry.e >> entry.i >>
        entry.lan >> entry.argperi >> entry.m;
    if (!iss) {
      std::cerr << "Error parsing line: " << i_line << '\n';
      continue;
    }
    entry.i *= M_PI / 180.0;
...
template <typename Real, typename Integer>
void KeplerianToCartesian(Keplerian<Real, Integer> const &kepl, Real &x,
                          Real &y, Real &z) {
  Real E{SolveKepler<Real, Integer>(kepl.m, kepl.e)};
  Real nu{2.0 * std::atan2(std::sqrt(1.0 + kepl.e) * std::sin(E / 2),
                           std::sqrt(1.0 - kepl.e) * std::cos(E / 2))};
  ...
  x = (cos_lan * cos_argperi - sin_lan * sin_argperi * cos_i) * xp +
      (-cos_lan * sin_argperi - sin_lan * cos_argperi * cos_i) * yp;
  y = (sin_lan * cos_argperi + cos_lan * sin_argperi * cos_i) * xp +
      (-sin_lan * sin_argperi + cos_lan * cos_argperi * cos_i) * yp;
...
void PropagateOrbit(Keplerian<Real, int> &kepl, Real t_end, Real t_ini = 0.0) {
  if (t_end == t_ini)
    return;
  Real n{std::sqrt(static_cast<Real>(GM) /
                   (kepl.a * kepl.a * kepl.a))}; // Mean motion (rad/day)
  kepl.m =
      std::fmod(kepl.m + n * (t_end - t_ini), 2.0 * M_PI); // Keep within [0,2π]
  if (kepl.m < 0)
    kepl.m += 2.0 * M_PI;
}
