...
// Main function
std::tuple<Real, Real, Real, Real, Real, Real, Real, Real>
ComputeChecks(int n_objects_t1, int n_objects_t2, int n_nodal_objs) {
  ...
  // Prepare the boxes 1
  std::unique_ptr<BoxUniquePtrList> boxes_1{
      std::make_unique<BoxUniquePtrList>()};
  boxes_1->reserve(n_objects_t1 - 1);
  for (Integer i{0}; i < n_objects_t1; ++i) {
    Vector x_1, y_1, z_1;
    Keplerian<Real, Integer> data_i1 = data_1[i];
    for (Integer j{0}; j < t_steps; ++j) {
      PropagateOrbit(data_i1, t_ini + j * dt,
  ...
  // Prepare the boxes 2
  std::unique_ptr<BoxUniquePtrList> boxes_2{
      std::make_unique<BoxUniquePtrList>()};
  boxes_2->reserve(n_objects_t2 - 1);
  for (Integer i{0}; i < n_objects_t2; ++i) {
    Vector x_2, y_2, z_2;
    Keplerian<Real, Integer> data_i2{data_2[i]};
    for (Integer j{0}; j < t_steps; ++j) {
      PropagateOrbit(data_i2, t_ini + j * dt,
                     (j == 0) ? t_ini : t_ini + (j - 1) * dt);
    ...
    for (Integer j{0}; j < t_steps - 1; ++j) {
      box_min_2(3 * j + 0) = std::min(x_2(j), x_2(j + 1)) - tol_trace;
      box_min_2(3 * j + 1) = std::min(y_2(j), y_2(j + 1)) - tol_trace;
      box_min_2(3 * j + 2) = std::min(z_2(j), z_2(j + 1)) - tol_trace;
      box_max_2(3 * j + 0) = std::max(x_2(j), x_2(j + 1)) + tol_trace;
      box_max_2(3 * j + 1) = std::max(y_2(j), y_2(j + 1)) + tol_trace;
      box_max_2(3 * j + 2) = std::max(z_2(j), z_2(j + 1)) + tol_trace;
    }
  ...
  return std::make_tuple(0.5 * checks_i_tt, 0.5 * checks_i_tb,
                         0.5 * checks_d_tt, 0.5 * checks_d_tb, 0.5 * time_i_tt,
                         0.5 * time_i_tb, 0.5 * time_d_tt, 0.5 * time_d_tb);
}
...
int main() {
  ...
  std::vector<std::string> colors = {"r", "g", "b", "c", "m", "y", "k",
                                     "r", "g", "b", "c", "m", "y", "k"};
  Integer n_times{5};
  std::vector<Integer> n_objects = {100,   177,   316,  562,   1000,
                                    1778,  3162,  5623, 10000, 17783,
                                    31623, 56234, 60000};
  std::vector<Integer> n_nodal_objs = {1, 10, 100, 1000, 10000};
  ...
  for (Integer j{0}; j < static_cast<Integer>(n_nodal_objs.size()); ++j) {
    for (Integer i{0}; i < static_cast<Integer>(n_objects.size()); ++i) {
      for (Integer k{0}; k < n_times; ++k) {
        auto [c_i_tt_tmp, c_i_tb_tmp, c_d_tt_tmp, c_d_tb_tmp, t_i_tt_tmp,
              t_i_tb_tmp, t_d_tt_tmp, t_d_tb_tmp] =
      ...
      std::cout << "Number of asteroids: " << n_objects[i] << std::endl
                ...
                << "----------------------------------------" << std::endl;
      checks_intersection << n_objects[i] << " " << n_nodal_objs[j] << " "
                          << checks_i_tt[i] << " " << checks_i_tb[i]
                          << std::endl;
      checks_distance << n_objects[i] << " " << n_nodal_objs[j] << " "
                      << checks_d_tt[i] << " " << checks_d_tb[i] << std::endl;
      time_intersection << n_objects[i] << " " << n_nodal_objs[j] << " "
                        << time_i_tt[i] << " " << time_i_tb[i] << std::endl;
      time_distance << n_objects[i] << " " << n_nodal_objs[j] << " "
                    << time_d_tt[i] << " " << time_d_tb[i] << std::endl;
    }
#ifdef AABBTREE_ENABLE_PLOTTING
    plot_checks_intersection.loglog(n_objects, checks_i_tt)
        ->line_width(1.5)
        .color(colors[j]);
    plot_checks_intersection.loglog(n_objects, checks_i_tb)
        ->line_width(1.5)
        .color(colors[j])
        .line_style("--");
    plot_checks_intersection.legend(L);
    plot_checks_distance.loglog(n_objects, checks_d_tt)
        ->line_width(1.5)
        .color(colors[j]);
    plot_checks_distance.loglog(n_objects, checks_d_tb)
        ->line_width(1.5)
        .color(colors[j])
        .line_style("--");
    plot_checks_distance.legend(L);
    plot_time_intersection.loglog(n_objects, time_i_tt)
        ->line_width(1.5)
        .color(colors[j]);
    plot_time_intersection.loglog(n_objects, time_i_tb)
        ->line_width(1.5)
        .color(colors[j])
        .line_style("--");
    plot_time_intersection.legend(L);
    plot_time_distance.loglog(n_objects, time_d_tt)
        ->line_width(1.5)
        .color(colors[j]);
    plot_time_distance.loglog(n_objects, time_d_tb)
        ->line_width(1.5)
        .color(colors[j])
        .line_style("--");
