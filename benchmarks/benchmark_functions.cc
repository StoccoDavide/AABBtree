...
template <typename Real> Real sine(Real const &x, Real const &f) {
  return std::sin(f * x * x);
}
template <typename Real>
Eigen::Vector<Real, Eigen::Dynamic>
sine(Eigen::Vector<Real, Eigen::Dynamic> const &x, Real const &f) {
...
int main() {
  ...
  VectorX y_f1(n_points);
  y_f1 = amplitude * sine<Real>(x_f1, 3.0 * M_PI);
  VectorX x_f2(n_points);
  x_f2 = amplitude * sine<Real>(y_f2, 3.0 * M_PI);
  ...
  timer.toc();
  std::cout << std::fixed << "Tree 1 built in " << timer.elapsed_us()
            << " us\n";
  timer.tic();
...
// Plot the functions
  ...
  bool do_intersect;
  timer.tic();
  do_intersect = tree_1.intersect(box, candidates_set);
  timer.toc();
  ...
  timer.tic();
  do_intersect = tree_2.intersect(box, candidates_set);
  timer.toc();
  ...
  timer.tic();
  do_intersect = tree_1.intersect(ray, candidates_set);
  timer.toc();
  ...
  timer.tic();
  do_intersect = tree_2.intersect(ray, candidates_set);
  timer.toc();
  ...
  timer.tic();
  do_intersect = tree_1.intersect(tree_2, candidates_map);
  timer.toc();
