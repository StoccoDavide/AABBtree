...
public:
  ...
  template <typename... Args> auto plot(Args... args) {
    return m_ax->plot(args...);
  }
  template <typename... Args> auto grid(Args... args) {
    return m_ax->grid(args...);
  }
  template <typename... Args> auto loglog(Args... args) {
    return m_ax->loglog(args...);
  }
  template <typename... Args> auto legend(Args... args) {
    return m_ax->legend(args...);
  }

  // Plot a box
  template <typename Real, Integer N>
  void plot_box(Box<Real, N> const &box, std::string const &color,
    ...
    if (m_ax) {
      m_ax->plot({min[0], max[0], max[0], min[0], min[0]},
  ...
  // Plot a tree
  template <typename Real, Integer N>
  void plot_tree(Tree<Real, N> const &tree, std::string const &color,
                 Real const line_width = 1.0) {
    static_assert(N == 2, "Plotting is only supported for 2D objects.");
  ...
  // Plot a point
  template <typename Real, Integer N>
  void plot_point(AABBtree::Vector<Real, N> const &point,
                  std::string const &color, Real const marker_size = 0.5) {
    static_assert(N == 2, "Plotting is only supported for 2D objects.");
  ...
  // Plot a segment
  template <typename Real, Integer N>
  void plot_segment(AABBtree::Vector<Real, N> const &p_1,
  ...
  void plot_ray(AABBtree::Ray<Real, N> const &ray, std::string const &color,
    ...
    static_assert(N == 2, "Plotting is only supported for 2D objects.");
    Real t{1000.0};
    this->plot_segment<Real, N>(
        ray.origin(), ray.origin() + t * ray.direction(), color, line_width);
  }

  // Plot a circle
  template <typename Real, Integer N>
  void plot_circle(Vector<Real, 2> const &center, Real const radius,
