 ...
 * vector. The ray extends infinitely in the direction of the vector from the
 * origin point. The class provides functionality for:
 ...
 * \note The direction vector is not required to be normalized, but some
 * operations (like distance calculations) may internally normalize it.
 */
template <typename Real, Integer N> class Ray {
  ...
  Vector m_direction; /**< Direction vector of the ray. */

public:
  /**
   * Default class destructor.
   *
   * \note This destructor is trivial and does not perform any special actions.
   */
  ~Ray() = default;
   ...
   *
   * Default class constructor that creates a ray with null origin and
   * direction.
   */
  Ray() = default;
  ...
  Ray(Real const o_x, Real const o_y, Real const o_z, Real const d_x,
      Real const d_y, Real const d_z)
  ...
  explicit Ray(Ray<OtherReal, N> const &r)
      : m_origin(r.m_origin.template cast<Real>()),
        m_direction(r.m_direction.template cast<Real>()) {}
   ...
   */
  template <typename NewReal> Ray<NewReal, N> cast() const {
    if constexpr (std::is_same<Real, NewReal>::value) {
      return *this;
    }
    return Ray<NewReal, N>(m_origin.template cast<NewReal>(),
                           m_direction.template cast<NewReal>());
  }
   ...
   */
  bool
  is_approx(Ray const &r,
            Real const tol = Eigen::NumTraits<Real>::dummy_precision()) const {
    return m_origin.isApprox(r.m_origin, tol) &&
           m_direction.isApprox(r.m_direction, tol);
  }
  ...
  Ray translated(Vector const &t) const {
    return Ray(m_origin + t, m_direction);
  }
   ...
   */
  template <typename Transform> Ray transformed(Transform const &t) const {
   ...
   * \tparam Transform Type of the transformation.
   */
  template <typename Transform> Ray &transform(Transform const &t) {
  ...
  bool contains(Point const &p,
    ...
    return std::abs(v.cross(m_direction).norm()) < tol &&
           v.dot(m_direction) >= -tol;
  }
   ...
   */
  bool intersects(Box<Real, N> const &b,
                  Real tol = Eigen::NumTraits<Real>::dummy_precision()) const {
    if (b.contains(m_origin)) {
      return true;
    }
    Point const &b_min{b.min()};
    Point const &b_max{b.max()};
    Vector t_min, t_max;
    t_min.setConstant(-std::numeric_limits<Real>::infinity());
    t_max.setConstant(std::numeric_limits<Real>::infinity());
   ...
   * \return True if the current ray intersects the given box, false otherwise.
   */
  bool intersect(Box<Real, N> const &b, Point &c, Point &f,
                 Real tol = Eigen::NumTraits<Real>::dummy_precision()) const {
    if (b.contains(m_origin)) {
      return true;
    }
   ...
   */
  Real
  squared_distance(Point const &p,
                   Real tol = Eigen::NumTraits<Real>::dummy_precision()) const {
    Real t{(p - m_origin).dot(m_direction) / m_direction.squaredNorm()};
    return (m_origin + std::max(static_cast<Real>(-tol), t) * m_direction - p)
        .squaredNorm();
  }
   ...
   */
  Real
  squared_distance(Point const &p, Point &c,
                   Real tol = Eigen::NumTraits<Real>::dummy_precision()) const {
    Real t{(p - m_origin).transpose().dot(m_direction) /
           m_direction.squaredNorm()};
   ...
   */
  Real distance(Point const &p,
                Real tol = Eigen::NumTraits<Real>::dummy_precision()) const {
    return std::sqrt(this->squared_distance(p, tol));
  }

  /**
   * Compute the distance between the current ray a given point, returning a
   ...
   * \return The distance between the ray and the point.
   */
  Real distance(Point const &p, Point &c,
                Real tol = Eigen::NumTraits<Real>::dummy_precision()) const {
  ...
  Real squared_interior_distance(
      ...
      Real tol = Eigen::NumTraits<Real>::dummy_precision()) const {
    Point p1, p2;
    return this->squared_interior_distance(b, p1, p2, tol);
  }

  /**
   * Compute the squared \em interior (or \em minimum) distance between the
   * current ray a given box, returning two points at the minimum distance.
   * \param[in] b Box to compute the squared distance to.
   ...
   */
  Real squared_interior_distance(
      Box<Real, N> const &b, Point &p1, Point &p2,
      Real tol = Eigen::NumTraits<Real>::dummy_precision()) const {
    if (b.contains(m_origin)) {
      return 0.0;
    }
    ...
    for (Integer i{0}; i < N; ++i) {
      if (m_origin[i] < b_min[i]) {
        p2[i] = b_min[i];
      } else if (m_origin[i] > b_max[i]) {
        p2[i] = b_max[i];
      } else {
        p2[i] = m_origin[i];
      }
  ...
  /**
   * Compute the \em interior (or \em minimum) distance between the current ray
   * a given box.
  ...
  Real interior_distance(
      ...
      Real tol = Eigen::NumTraits<Real>::dummy_precision()) const {
    return std::sqrt(
        static_cast<Real>(this->squared_interior_distance(b, p1, p2, tol)));
  }
  ...
  Real squared_exterior_distance(
      ...
      Real tol = Eigen::NumTraits<Real>::dummy_precision()) const {
    Point p1, p2;
    return this->squared_exterior_distance(b, p1, p2, tol);
  }

  /**
   * Compute the squared \em exterior (or \em maximum) distance between the
   * current ray a given box, returning two points at the maximum distance.
   * \param[in] b Box to compute the squared distance to.
  ...
  /**
   * Compute the \em exterior (or \em maximum) distance between the current ray
   * a given box.
   * \param[in] b Box to compute the distance to.
   * \param[in] tol Tolerance to use for the distance computation.
   * \return The distance between the ray and the box.
  ...
  void print(std::ostream &os) const {
    os << "RAY INFO" << std::endl
       << "\to = " << m_origin.transpose() << std::endl
       << "\td = " << m_direction.transpose() << std::endl;
  }
