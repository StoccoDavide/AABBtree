...
/**
 * \brief A class representing an axis-aligned bounding box (AABB) in
 * N-dimensional space.
 *
 * This class represents an axis-aligned bounding box (AABB) defined by two
 * opposite corners:
 ...
 * \tparam N Dimension of the ambient space.
 * \note The box is considered empty if any component of the minimum corner is
 * greater than the corresponding component of the maximum corner.
...
template <typename Real, Integer N> class Box {
  ...
  Point
      m_min; /**< Minimal corner of the box (lower bounds in all dimensions). */
  Point
      m_max; /**< Maximal corner of the box (upper bounds in all dimensions). */
...
public:
  ...
  /**
   * Default constructor creating an empty box whether the minimal and maximal
   * corners are set to the maximum and minimum representable values,
   * respectively.
   */
  Box() { this->set_empty(); }
  ...
  /**
   * Class constructor for a axis-aligned box given the minimal and maximal
   * corners.
   * \param[in] t_min Minimal corner of the box.
   * \param[in] t_max Maximal corner of the box.
   * \warning If the reordering is not performed, and the minimal corner is
   * greater than the maximal corner in any dimension, undefined behavior may
   * occur.
   */
  ...
  Box(Real const x_min, Real const y_min, Real const z_min, Real const x_max,
      Real const y_max, Real const z_max)
      : m_min(x_min, y_min, z_min), m_max(x_max, y_max, z_max) {}

  /**
   * Copy constructor for a axis-aligned box given another box with a different
   * scalar type.
   * \param[in] b Box to copy.
   ...
   * \note If the new real type is equal to the current scalar type currently
   * used, then this function returns a const reference to the current object.
   */
  template <typename NewReal> Box<NewReal, N> cast() const {
  ...
  /**
   * Check if the box is empty, i.e., the minimal corner is greater than the
   * maximal corner in any dimension.
   ...
   */
  bool
  is_approx(Box const &b,
            Real const tol = Eigen::NumTraits<Real>::dummy_precision()) const {
  ...
  Integer longest_axis() const {
    ...
    return std::distance(sizes.data(),
                         std::max_element(sizes.data(), sizes.data() + N));
  }
  ...
  void sort_axes_length(Vector &sizes, Eigen::Vector<Integer, N> &ipos) const {
    ...
    std::sort(ipos.data(), ipos.data() + N,
              [&sizes](Integer i, Integer j) { return sizes[i] > sizes[j]; });
  }
   ...
   */
  void set_empty() {
    m_min.setConstant(+std::numeric_limits<Real>::infinity());
    m_max.setConstant(-std::numeric_limits<Real>::infinity());
  }
  ...
  bool intersects(Box const &b) const {
    // return (m_min.array() <= b.m_max.array()).all() && (b.m_min.array() <=
    // m_max.array()).all();
   ...
   * \return True if boxes intersect, false otherwise.
   */
  template <Integer d = 2> bool quasi_intersects(Box const &b) const {
    return (m_min.template head<d>().array() <=
            b.m_max.template head<d>().array())
               .all() &&
           (b.m_min.template head<d>().array() <=
            m_max.template head<d>().array())
               .all();
  }
   ...
   * \note The resulting box may be larger than the original due to
   * axis-alignment.
   */
  template <typename Transform> Box transformed(Transform const &t) const {
   ...
   * axis-alignment.
   */
  template <typename Transform> Box &transform(Transform const &t) {
    m_min.transform(t);
    m_max.transform(t);
    this->reorder();
    return *this;
   ...
   * \return Side::LEFT if max[dim] < x+tol, Side::RIGHT if x-tol < min[dim],
   * Side::INSIDE otherwise.
   */
  Side which_side(Real const x, Real const tol, Integer const dim) const {
    Real const x_min{m_min(dim)};
    Real const x_max{m_max(dim)};
    bool const on_left{x_max < x + tol};
    bool const on_right{x - tol < x_min};
    if (on_left && on_right) {
      return 0.5 * (x_min + x_max) < x ? Side::LEFT : Side::RIGHT;
    } else if (on_left) {
      return Side::LEFT;
    } else if (on_right) {
      return Side::RIGHT;
   ...
   * \note Equivalent to \c contains() for points.
   */
  template <Integer d = 2> bool quasi_intersects(Point const &p) const {
   ...
   */
  template <typename Derived> Box &extend(Point const &p) {
   ...
   * \note The returned value is positive if the point is outside the box, zero
   * otherwise.
   */
  Real squared_interior_distance(Point const &p) const {
    ...
    for (Integer i{0}; i < N; ++i) {
      if (m_min[i] > p[i]) {
        Real aux{m_min[i] - p[i]};
        dist2 += aux * aux;
      } else if (p[i] > m_max[i]) {
        Real aux{p[i] - m_max[i]};
        dist2 += aux * aux;
      }
    }
    return dist2;
  ...
  /**
   * Compute the \em interior (or \em minimum) distance between the current box
   * a given point.
   * \param[in] p Point to compute the distance to.
   * \return The distance between the box and the point.
   * \note The distance is positive if the point is outside the box, zero
   * otherwise.
   */
  ...
  /**
   * Compute the \em interior (or \em minimum) distance between the current box
   * a given point, returning a point at the given distance.
   * \param[in] p Point to compute the distance to.
   * \param[out] c Closest point to the box.
   * \return The distance between the box and the point.
   * \note The returned value is positive if the point is outside the box, zero
  ...
  /**
   * Compute the squared \em exterior (or \em maximum) distance between the
   * current box a  given point.
   * \param[in] p Point to compute the squared distance to.
   * \return The squared distance between the box and the point.
   * \note The returned value is positive if the point is outside the box, zero
  ...
  /**
   * Compute the squared \em exterior (or \em maximum) distance between the
   * current box a given point, returning a point at the given distance.
   * \param[in] p Point to compute the squared distance to.
  ...
  /**
   * Compute the \em exterior (or \em maximum) distance between the current box
   * a given point.
   * \param[in] p Point to compute the distance to.
   * \return The distance between the box and the point.
   * \note The distance is positive if the point is outside the box, zero
  ...
  /**
   * Compute the squared \em interior (or \em minimum) distance between the
   * current box a given box.
   * \param[in] b Box to compute the squared distance to.
   * \return The squared distance between the boxes.
   * \note The squared distance is positive if the boxes do not intersect, zero
   * otherwise.
   */
  ...
  /**
   * Compute the squared \em interior (or \em minimum) distance between the
   * current box a given box, returning two points at the minimum distance.
   * \param[in] b Box to compute the squared distance to.
  ...
  Real squared_interior_distance(Box const &b, Point &p1, Point &p2) const {
    ...
    for (Integer i{0}; i < N; ++i) {
      if (m_min[i] > b.m_max[i]) {
        p1[i] = b.m_max[i];
        p2[i] = m_min[i];
      } else if (b.m_min[i] > m_max[i]) {
        p1[i] = m_max[i];
        p2[i] = b.m_min[i];
      } else {
        p1[i] = p2[i] = 0.5 * (std::min(m_max[i], b.m_max[i]) +
                               std::max(m_min[i], b.m_min[i]));
      }
    }
   ...
   * \note The distance is positive if the boxes do not intersect, zero
   * otherwise.
   */
  Real interior_distance(Box const &b) const {
    return std::sqrt(this->squared_interior_distance(b));
  }

  /**
   * Compute the \em interior (or \em minimum) distance between the current box
   * a given box, returning two points at the minimum distance.
   * \param[in] b Box to compute the distance to.
   * \param[out] p1 First point at the minimum distance.
   * \param[out] p2 Second point at the minimum distance.
   * \return The distance between the boxes.
   ...
   * \param[in] b Box to compute the squared distance to.
   * \return The squared distance between the boxes.
   * \note The squared distance is positive if one box is not contained in the
   * other, zero otherwise.
   */
  Real squared_exterior_distance(Box const &b) const {
    Real dist2{Real(0)};
    for (Integer i{0}; i < N; ++i) {
      Real const aux{std::max(m_max[i], b.m_max[i]) -
                     std::min(m_min[i], b.m_min[i])};
      dist2 += aux * aux;
    }
    return dist2;
  }
  ...
  /**
   * Compute the squared \em exterior (or \em maximum) distance between the
   * current box a given box, returning two points at the maximum distance.
   * \param[in] b Box to compute the squared distance to.
  ...
  /**
   * Compute the \em exterior (or \em maximum) distance between the current box
   * a given box, and return two points at the maximum distance.
   * \param[in] b Box to compute the distance to.
   * \param[out] p1 First point at the maximum distance.
   * \param[out] p2 Second point at the maximum distance.
   * \return The distance between the boxes.
   ...
   */
  bool intersects(Ray<Real, N> const &r,
                  Real tol = Eigen::NumTraits<Real>::dummy_precision()) const {
   ...
   * \return True if the current box intersects the given ray, false otherwise.
   */
  bool intersect(Ray<Real, N> const &r, Point &c, Point &f,
                 Real tol = Eigen::NumTraits<Real>::dummy_precision()) const {
   ...
   * \note The squared distance is positive if the ray and the box do not
   * intersect, zero otherwise.
   */
  Real squared_interior_distance(
      Ray<Real, N> const &r,
      Real tol = Eigen::NumTraits<Real>::dummy_precision()) const {
  ...
  /**
   * Compute the squared \em interior (or \em minimum) distance between the
   * current box a given ray, returning two points at the minimum distance.
   * \param[in] r Ray to compute the squared distance to.
   ...
   */
  Real squared_interior_distance(
      Ray<Real, N> const &r, Point &p1, Point &p2,
      Real tol = Eigen::NumTraits<Real>::dummy_precision()) const {
   ...
   * \note The distance is positive if the ray and the box do not intersect,
   * zero otherwise.
   */
  Real interior_distance(
      Ray<Real, N> const &r,
      Real tol = Eigen::NumTraits<Real>::dummy_precision()) const {
  ...
  /**
   * Compute the \em interior (or \em minimum) distance between the current box
   * a given ray, returning two points at the minimum distance.
   * \param[in] r Ray to compute the distance to.
   * \param[out] p1 First point at the minimum distance (on the current box).
   * \param[out] p2 Second point at the minimum distance (on the ray).
   * \param[in] tol Tolerance to use for the distance computation.
   ...
   * other, zero otherwise.
   */
  Real squared_exterior_distance(
      Ray<Real, N> const &r,
      Real tol = Eigen::NumTraits<Real>::dummy_precision()) const {
  ...
  /**
   * Compute the squared \em exterior (or \em maximum) distance between the
   * current box a given ray, returning two points at the maximum distance.
   * \param[in] r Ray to compute the squared distance to.
  ...
  /**
   * Compute the \em exterior (or \em maximum) distance between the current box
   * a given ray, and return two points at the maximum distance.
   * \param[in] r Ray to compute the distance to.
   * \param[out] p1 First point at the maximum distance (on the current box).
   * \param[out] p2 Second point at the maximum distance (on the ray).
   * \param[in] tol Tolerance to use for the distance computation.
  ...
  void print(std::ostream &os) const {
    os << "BOX INFO" << std::endl
       << "\tmin = " << m_min.transpose() << std::endl
       << "\tmax = " << m_max.transpose() << std::endl;
  }
