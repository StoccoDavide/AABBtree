...
namespace AABBtree {
  ...
  */
  ...
  /**
    ...
    */
    template <typename T = Real>
    Box<T, 2>(Real const x_min, Real const y_min, Real const x_max, Real const y_max)
    ...
    */
    template <typename T = Real>
    Box<T, 3>(Real const x_min, Real const y_min, Real const z_min, Real const x_max, Real const y_max,
    ...
    */
    ...
    /**
    */
    Real length(Integer i) const {return this->m_max(i) - this->m_min(i);}
    ...
    Integer longest_axis(Real & max_length, Real & mid_point) const
    {
      ...
      for (Integer i{0}; i < N; ++i) {
        if (Real const tmp_length{this->m_max(i) - this->m_min(i)}; max_length < tmp_length)
          {max_length = tmp_length; mid_point = (this->m_max(i) + this->m_min(i))/2.0; axis = i;}
      }
      return axis;
    }
    ...
    * \return The barycenter of the box.
    */
    Point barycenter() const {return 0.5*(this->m_min + this->m_max);}

    /**
    * Compute the barycenter of the box.
    * \return The barycenter of the box.
    */
    Real barycenter(Integer i) const {return 0.5*(this->m_min(i) + this->m_max(i));}
    ...
    */
    ...
    {
      Box result(this->m_min, this->m_max);
      result.translate(t);
      return result;
    }
    ...
    * \note The returned value is positive if the point is outside the box, zero otherwise.
    */
    Real squared_distance(Point const & p, Point & p_min, Point & p_max) const
    {
      if (this->contains(p)) {return 0.0;}
    ...
    * \note The returned value is positive if the point is outside the box, zero otherwise.
    */
    Real distance(Point const & p, Point & p_min, Point & p_max) const
    {return std::sqrt(this->squared_distance(p, p_min, p_max));}
    ...
    * \note The squared distance is positive if the boxes do not intersect, zero otherwise.
    */
    Real squared_interior_distance(Box const & b, Point & p_min, Point & p_max) const
    {
      if (this->intersects(b)) {return 0.0;}
      for (Integer i{0}; i < N; ++i) {
        if (this->m_min[i] > b.m_max[i]) {p_min[i] = b.m_max[i]; p_max[i] = this->m_min[i];}
        else if (b.m_min[i] > this->m_max[i]) {p_min[i] = this->m_max[i]; p_max[i] = b.m_min[i];}
        else {p_min[i] = p_max[i] = 0.5*(std::min(this->m_max[i], b.m_max[i]) + std::max(this->m_min[i], b.m_min[i]));}
      }
      return (p_max - p_min).squaredNorm();
    }
    ...
    * \note The distance is positive if the boxes do not intersect, zero otherwise.
    */
    Real interior_distance(Box const & b, Point & p_min, Point & p_max) const
    ...
    * \return The squared distance between the boxes.
    */
    Real squared_exterior_distance(Box const & b, Point & p_min, Point & p_max) const
    {
    ...
    * \note The distance is positive if the boxes do not intersect, zero otherwise.
    */
    Real exterior_distance(Box const & b, Point & p_min, Point & p_max) const
    {return std::sqrt(this->squared_exterior_distance(b, p_min, p_max));}
