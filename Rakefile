...
if ENABLE_PLOTTING then
  cmd_cmake_build += "-DAABBTREE_ENABLE_PLOTTING:VAR=true "
else
  cmd_cmake_build += "-DAABBTREE_ENABLE_PLOTTING:VAR=false "
end
...
task :build_gen do
  ...
  puts "run CMake for AABBtree".yellow
  sh cmd_cmake + ' ..'
  puts "compile with CMake for AABBtree".yellow
  if BUILD_DEBUG then
