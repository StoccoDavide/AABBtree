...
TEMPLATE_TEST_CASE("Tree-Point-Ray)", "[template]", float, double) {
...
#ifdef AABBTREE_ENABLE_PLOTTING
    P.plot_segment<TestType>(segments[i].point(0), segments[i].point(1),
                             colors[6], 1.0);
#endif
  }
  ...
  // Segments boxes
  std::unique_ptr<BoxUniquePtrList> boxes =
  ...
  for (Integer i{0}; i < n; ++i) {
    ...
    if (tree.intersect(segments[i].bounding_box(), candidates)) {
      ...
      for (Integer i{0}; i < static_cast<Integer>(candidates_vec.size()); ++i) {
        for (Integer j{i + 1}; j < static_cast<Integer>(candidates_vec.size());
             ++j) {
          if (segments[candidates_vec[i]].intersect(segments[candidates_vec[j]],
                                                    point)) {
  ...
  auto distance_func = [](Point const &p, Box const &b) -> TestType {
...
#endif
  TestType const distance_within{
      tree.within_distance(point, max_distance, candidates, distance_func)};
